...
function openNavMenu(section) {
  window.dispatchEvent(
    new CustomEvent("wt-open-nav-menu", { detail: { section } })
  );
}
...
export default function MobileBottomNav() {
  ...
  const menuParam = searchParams.get("menu")?.toLowerCase();
  const domesticActive =
    pathname === "/" && (packageParam === "domestic" || menuParam === "domestic");
  ...
  const handlePackageTab = (section) => {
    if (pathname === "/") {
      router.replace(`/?menu=${section}`, { scroll: false });
      openNavMenu(section);
      return;
    }
  ...
  return (
    <nav className="mobileBottomNav" aria-label="Mobile navigation">
      ...
      <button
        ...
        aria-expanded={menuParam === "domestic"}
        onClick={() => handlePackageTab("domestic")}
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
          <path
      ...
      <button
        type="button"
        className={`mobileNavTab${internationalActive ? " navActive" : ""}`}
        aria-expanded={menuParam === "international"}
        onClick={() => handlePackageTab("international")}
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
