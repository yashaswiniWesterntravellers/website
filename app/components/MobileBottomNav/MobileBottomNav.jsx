...
import { PACKAGE_MENU_CONFIG } from "../../data/navPackages";
...
export default function MobileBottomNav() {
  ...
  const domesticActive = pathname === "/" && (packageParam === "domestic" || packageMenu === "domestic");
  const internationalActive =
    pathname === "/" && (packageParam === "international" || packageMenu === "international");
  ...
  const closeMenu = useCallback(() => setPackageMenu(null), []);

  const openMenu = (type) => {
    setPackageMenu((current) => (current === type ? null : type));
  };

  useEffect(() => {
    if (!packageMenu) return undefined;
  ...
  }, [packageMenu, closeMenu]);

  const menuConfig = packageMenu ? PACKAGE_MENU_CONFIG[packageMenu] : null;
    ...
    <>
      {menuConfig ? (
        <>
          <button
            type="button"
            className={styles.packageMenuBackdrop}
            aria-label="Close package menu"
            onClick={closeMenu}
          />
          <div
            className={styles.packageMenuSheet}
            role="dialog"
            aria-modal="true"
            aria-labelledby="package-menu-title"
          >
            <div className={styles.packageMenuHeader}>
              <h2 id="package-menu-title" className={styles.packageMenuTitle}>
                {menuConfig.title}
              </h2>
              <button
                type="button"
                className={styles.packageMenuClose}
            ...
            <ul className={styles.packageMenuList}>
              {menuConfig.links.map((item) => (
                <li key={item.label} className={styles.packageMenuItem}>
                  <Link
                    href={item.href}
                    className={styles.packageMenuLink}
                    onClick={closeMenu}
                  >
                    {item.label}
                  </Link>
            ...
            <div className={styles.packageMenuFooter}>
              <Link
                href={menuConfig.viewAllHref}
                className={styles.packageMenuViewAll}
                onClick={closeMenu}
              >
                {menuConfig.viewAllLabel}
              </Link>
            </div>
          </div>
        </>
      ...
      <nav className="mobileBottomNav" aria-label="Mobile navigation">
        ...
        <button
          type="button"
          className={`${styles.navTabButton} ${domesticActive ? "navActive" : ""}`}
          aria-expanded={packageMenu === "domestic"}
          aria-haspopup="dialog"
          onClick={() => openMenu("domestic")}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <button
          type="button"
          className={`${styles.navTabButton} ${internationalActive ? "navActive" : ""}`}
          aria-expanded={packageMenu === "international"}
          aria-haspopup="dialog"
          onClick={() => openMenu("international")}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
